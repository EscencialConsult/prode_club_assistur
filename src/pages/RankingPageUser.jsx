        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,paddingTop:12}}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              gap:10,
              padding:'9px 12px',
              background: u.user_id === user?.id ? 'rgba(78,140,255,.1)' : '#fff',
              border: u.user_id === user?.id ? '1.5px solid #4e8cff' : '1px solid #f5f3ee',
              boxShadow: u.user_id === user?.id ? '0 0 0 1px rgba(78,140,255,.3), 0 2px 8px rgba(78,140,255,.12)' : 'none',
              borderRadius:10,
              alignItems:'center',
              transition:'all .15s',
            }}
              ...
              <div style={{minWidth:0}}>
                <p style={{fontWeight: u.user_id === user?.id ? 700 : 500,fontSize:11,color: u.user_id === user?.id ? '#4e8cff' : '#0c182b',margin:'0 0 1px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                  {u.nombre}
                  {u.user_id === user?.id && (
                    <span style={{
                      fontSize:8,
