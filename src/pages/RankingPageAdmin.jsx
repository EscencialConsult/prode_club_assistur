...
.rk-detail-btn-podio.active { background:#0c182b;color:#4e8cff }
...
.rk-detail-btn-mini.active { background:#0c182b;color:#4e8cff }
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'2rem 1.5rem 3rem' }}>
        ...
        <div className="rk-in" style={{ marginBottom:'1.5rem' }}>
          <h1 style={{
            ...
            <span style={{color:'#4e8cff'}}>ADMIN</span>
          </h1>
...
function BetRow({ bet, sel, onPick }) {
  ...
  return (
    <div className={`rk-row${sel?' sel':''}`} onClick={()=>onPick(bet)}>
      ...
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={sel?'#4e8cff':'#c8d0dc'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function PrediccionRow({ pred, apuesta }) {
  ...
  if (puntosDlBackend !== null) {
    ...
    if (puntosDlBackend === ptsExacto) {
      ...
      borderC='#4e8cff40'; bgC='#4e8cff0a'; badge={c:'#4e8cff',label:`+${puntosDlBackend}`}
    ...
    else if (dif) { borderC='#4e8cff40'; bgC='#4e8cff0a'; badge={c:'#4e8cff',label:`+${ptsDif}`} }
          ...
          {otros.map((u, idx) => {
            ...
            return (
              <div key={u.user_id} style={{
                background: isMe ? 'rgba(78,140,255,.1)' : '#fff',
                border: isMe ? '1.5px solid #4e8cff' : '1px solid #f5f3ee',
                boxShadow: isMe ? '0 0 0 1px rgba(78,140,255,.3), 0 2px 8px rgba(78,140,255,.12)' : 'none',
                borderRadius:10,
                overflow:'hidden',
                transition:'all .15s',
              }}>
                ...
                <div style={{
                  ...
                  <div style={{minWidth:0}}>
                    <p style={{fontWeight: isMe ? 700 : 500,fontSize:11,color: isMe ? '#4e8cff' : '#0c182b',margin:'0 0 1px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                      {u.nombre}
                      {isMe && (
                        <span style={{
                          fontSize:8,
