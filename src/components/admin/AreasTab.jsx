...
function BtnGold({ children, onClick, disabled, type = 'button' }) {
  return (
    <button
      ...
      style={{ background: '#0c182b', color: '#4e8cff', boxShadow: '0 4px 14px rgba(12,24,43,.2)' }}
...
function AdminInput({ value, onChange, placeholder, required, type = 'text' }) {
  return (
    <input
      ...
      onFocus={e => { e.target.style.borderColor = '#4e8cff'; e.target.style.boxShadow = '0 0 0 3px rgba(78,140,255,.12)' }}
...
//             {savingArea
//               ? <><span className="w-4 h-4 rounded-full border-2 border-t-transparent inline-block animate-spin" style={{ borderColor: '#4e8cff' }} /> Creando...</>
//               : 'Crear área'
//             }
//           </BtnGold>
//         </form>
...
//             <span className="font-display text-xl" style={{ color: '#4e8cff' }}>({areasAll.length})</span>
//           </div>
...
//             <span className="inline-block w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: '#4e8cff' }} />
