...
const ESTADO_COLORS = {
  ...
  finalizado:  { color: '#4e8cff', bg: 'rgba(78,140,255,.1)',   border: 'rgba(78,140,255,.3)',   label: 'Finalizado' },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {match.fase && (
          <p className="font-body text-[10px] text-center mb-4 uppercase tracking-widest" style={{ color: elim ? '#4e8cff' : '#a8b2c4' }}>
        ...
        <div className="flex items-center gap-4 mb-5">
          <div className="flex-1 text-center">
            ...
            <input
              ...
              style={{ background: '#faf7f0', border: '1px solid #e8dfd0', color: '#4e8cff' }}
              onFocus={e => { e.target.style.borderColor = 'rgba(78,140,255,.5)'; e.target.style.boxShadow = '0 0 0 3px rgba(78,140,255,.1)' }}
        ...
        {debeMostrarPenales && (
          <div className="mb-5 p-3 rounded-xl" style={{ background: 'rgba(78,140,255,.06)', border: '1px solid rgba(78,140,255,.25)' }}>
            <div className="flex items-center gap-2 mb-2">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4e8cff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
              </svg>
              <p className="font-body text-xs uppercase tracking-widest font-bold" style={{ color: '#4e8cff' }}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            style={{ background: 'linear-gradient(135deg,#4e8cff,#0041f0)', color: '#05090f', boxShadow: '0 4px 16px rgba(78,140,255,.3)' }}
            onMouseEnter={e => { if (!saving) e.currentTarget.style.boxShadow = '0 6px 24px rgba(78,140,255,.5)' }}
            onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 16px rgba(78,140,255,.3)' }}>
    ...
    <div className="animate-fade-in delay-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          <h2 className="font-display text-2xl text-white" style={{ letterSpacing: '.02em' }}>
            PARTIDOS <span style={{ color: '#4e8cff' }}>({matches.length})</span>
          </h2>
          <div className="flex items-center gap-3">
            {liveCount > 0 && (
        ...
        <button
          ...
          style={{ background: 'rgba(78,140,255,.1)', border: '1px solid rgba(78,140,255,.3)', color: '#4e8cff' }}
          onMouseEnter={e => { if (!syncing) { e.currentTarget.style.background = '#4e8cff'; e.currentTarget.style.color = '#05090f' } }}
          onMouseLeave={e => { e.currentTarget.style.background = 'rgba(78,140,255,.1)'; e.currentTarget.style.color = '#4e8cff' }}
        >
          {syncing
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {['todas', ...fasesDisponibles].map(f => (
              <button key={f} type="button"
                ...
                style={{
                  fontSize: 11,
                  background: filtroFase === f ? '#4e8cff' : 'rgba(255,255,255,.06)',
                  border: `1px solid ${filtroFase === f ? '#4e8cff' : 'rgba(255,255,255,.12)'}`,
          ...
          {['todos', 'programado', 'en_vivo', 'finalizado'].map(est => {
            ...
            return (
              <button key={est} type="button"
                ...
                style={{
                  fontSize: 11,
                  background: isActive ? (s?.bg || '#4e8cff') : 'rgba(255,255,255,.06)',
                  border: `1px solid ${isActive ? (s?.border || '#4e8cff') : 'rgba(255,255,255,.12)'}`,
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl p-12 text-center"
          style={{ background: 'rgba(66,91,139,.08)', border: '1px dashed rgba(78,140,255,.2)' }}>
          ...
          {filtered.map(match => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(78,140,255,.25)'; e.currentTarget.style.background = 'linear-gradient(155deg,rgba(66,91,139,.22),rgba(66,91,139,.08))' }}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col gap-0.5 flex-shrink-0 w-24 hidden sm:flex">
                    {faseLabel && (
                      <span className="text-[9px] font-semibold uppercase tracking-wider font-body" style={{ color: elim ? '#4e8cff' : '#a8b2c4' }}>
                        {faseLabel}
                      </span>
                    )}
                    {match.grupo && (
                  ...
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex-shrink-0 px-3 py-1 rounded-lg text-center"
                      ...
                      {isScheduled ? (
                        ...
                        <>
                          <span className="font-display text-base" style={{ color: isLive ? '#ff4d6d' : '#4e8cff' }}>
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2">
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(78,140,255,.1)', border: '1px solid #f0eadb', color: '#4e8cff' }}
                      onMouseEnter={e => { e.currentTarget.style.background = '#4e8cff'; e.currentTarget.style.color = '#05090f' }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(78,140,255,.1)'; e.currentTarget.style.color = '#4e8cff' }}
