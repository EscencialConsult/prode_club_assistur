  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        onFocus={e => {
          e.target.style.borderColor = error ? '#e03252' : '#4e8cff'
          e.target.style.boxShadow = error 
            ? '0 0 0 3px rgba(224,50,82,.08)' 
            : '0 0 0 3px rgba(78,140,255,.08)'
        }}
        onBlur={e => {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <span className="font-body text-xs" style={{ color: '#a8b2c4' }}>
            <span className="font-bold" style={{ color: seleccionados > 0 ? '#0041f0' : '#a8b2c4' }}>
              {seleccionados}
            </span>
        ...
        <input
          ...
          onFocus={e => { 
            e.target.style.borderColor = '#4e8cff'
            e.target.style.boxShadow = '0 0 0 3px rgba(78,140,255,.08)' 
          }}
          onBlur={e => { 
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors disabled:opacity-40 font-semibold" style={{ color: '#0041f0' }}
            onMouseEnter={e => { if (partidosFiltrados.length > 0) e.currentTarget.style.color = '#4e8cff' }}
              ...
              return (
                <div key={`${gr.fase}-${gr.jornada}-${gr.grupo}`}>
                  ...
                  <div className="sticky top-0 z-10 px-4 py-2.5 flex items-center justify-between"
                    style={{ background: 'linear-gradient(135deg, #0a1226 0%, #1a2540 100%)', borderBottom: '1px solid rgba(78,140,255,0.15)' }}>
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-md flex items-center justify-center font-display font-bold text-xs"
                        style={{ background: '#4e8cff', color: '#0a1226' }}>
                        {gr.partidos.length}
                      </span>
                  ...
                  {gr.partidos.map(m => {
                    ...
                    return (
                      <label
                        ...
                        style={{
                          background: checked ? 'rgba(78,140,255,0.08)' : '#fff',
                          borderBottom: '1px solid #f5f5f5',
                          opacity: yaTermino ? 0.5 : 1,
                          cursor: yaTermino ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={e => { 
                          if (!checked && !yaTermino) e.currentTarget.style.background = 'rgba(78,140,255,0.04)' 
                        }}
                        onMouseLeave={e => { 
                      ...
                      >
                        ...
                        <span className="flex-shrink-0 w-5 h-5 rounded flex items-center justify-center transition-all"
                          style={{
                            border: `2px solid ${checked ? '#4e8cff' : '#e8dfd0'}`,
                            background: checked ? '#4e8cff' : '#fff',
                          }}>
                          {checked && (
      ...
      {isPro && form.type === 'grupos' && (
        <div className="flex flex-col gap-3 p-4 rounded-xl"
          style={{ border: '1.5px solid rgba(78,140,255,.2)', background: 'rgba(78,140,255,.04)' }}>
          <span className="font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#0041f0' }}>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {primerPartido && !errorFecha && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg"
              style={{ background: 'rgba(78,140,255,0.06)', border: '1px solid rgba(78,140,255,0.15)' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0041f0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5">
