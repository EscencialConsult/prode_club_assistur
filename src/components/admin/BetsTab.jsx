      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            {bet.premio && (
              <span className="flex items-center gap-1 text-xs font-body" style={{ color: '#0041f0' }}>
...
<div className="flex flex-row sm:flex-col gap-1.5 flex-shrink-0 w-full sm:w-auto">
  ...
  {bet.estado === 'cerrada' && onFinalize && (
    <button
      ...
      className="inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-full font-body font-semibold transition-all w-full sm:w-auto"
      style={{ fontSize: 11, background: 'rgba(78,140,255,.1)', border: '1px solid rgba(78,140,255,.35)', color: '#0041f0' }}
      onMouseEnter={e => { e.currentTarget.style.background = '#4e8cff'; e.currentTarget.style.color = '#05090f' }}
      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(78,140,255,.1)'; e.currentTarget.style.color = '#0041f0' }}
    >
      Finalizar
    </button>
  )}
...
>
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
            style={{ background: 'rgba(78,140,255,.12)', border: '1px solid rgba(78,140,255,.25)' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0041f0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex flex-col gap-5 w-full">
        ...
        {loading && bets.length === 0 && (
          <div className="text-center py-16">
            <span className="inline-block w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#4e8cff' }} />
        ...
        {!loading && bets.length === 0 && (
          <div className="rounded-2xl p-12 text-center" style={{ background: '#fff', border: '1px dashed #e8dfd0' }}>
            <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4"
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0041f0" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {finishedBets.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-display text-lg" style={{ color: '#0041f0' }}>({finishedBets.length})</span>
            </div>
            <div className="flex flex-col gap-3">
