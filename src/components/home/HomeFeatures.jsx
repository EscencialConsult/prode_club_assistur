      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-end mb-14">
          <div>
            ...
            <h2 className="font-display text-white" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', lineHeight: 1, letterSpacing: '.01em' }}>
              UNA PLATAFORMA<br /><span style={{ color: '#4e8cff' }}>COMPLETA</span>
            </h2>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map(({ title, desc, icon }) => (
            <div key={title} className="rounded-xl p-5 transition-all duration-300"
              style={{ background: 'linear-gradient(155deg,rgba(66,91,139,.22),rgba(66,91,139,.05))', border: '1px solid rgba(78,140,255,.18)' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#4e8cff'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 20px 48px rgba(0,0,0,.4)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(78,140,255,.18)'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '' }}
            >
        ...
        <div className="mt-12 text-center">
          <Link to="/register"
            ...
            style={{ background: '#4e8cff', color: '#05090f', boxShadow: '0 8px 24px rgba(78,140,255,.25)', textDecoration: 'none' }}
