const ITEMS = [
  ...
  {
    bg: 'rgba(78,140,255,.16)', stroke: '#0041f0',
...
export default function HomeStrip() {
  return (
    <>
      ...
      {/* Marquee */}
      <div className="marquee-wrap-el" style={{ background: '#f0eadb', borderTop: '1px solid rgba(78,140,255,.3)', borderBottom: '1px solid rgba(78,140,255,.3)', padding: '.9rem 0' }}>
        <div className="marquee-track-el font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#0c182b' }}>
          {[...MARQUEE, ...MARQUEE].map((item, i) => (
            <span key={i} className="flex items-center gap-2.5 whitespace-nowrap">
              <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#4e8cff' }} />
              {item}
            </span>
          ))}
        </div>
