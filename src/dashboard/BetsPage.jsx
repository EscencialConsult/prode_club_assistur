...
const STATE={
  ...
  finalizada:{label:'FINALIZADA',color:'#0041f0',bg:'rgba(78,140,255,.08)',border:'rgba(78,140,255,.22)'},
}
...
function Toast({msg,ok}){
  if(!msg)return null
  return(
    <div style={{position:'fixed',top:'1.2rem',right:'1.2rem',zIndex:999,padding:'.75rem 1.1rem',borderRadius:12,background:ok?'#0c182b':'#fff',border:`1px solid ${ok?'rgba(78,140,255,.4)':'rgba(224,50,82,.3)'}`,boxShadow:'0 8px 28px rgba(12,24,43,.15)',display:'flex',alignItems:'center',gap:'.55rem',fontFamily:"'DM Sans',sans-serif",fontSize:'.84rem',fontWeight:600,color:ok?'#4e8cff':'#e03252',animation:'tin .3s ease both'}}>
      {ok
  ...
  return(
    <div style={{...CARD_BASE,borderRadius:18,padding:'1.4rem 1.5rem',border:`1px solid ${live?'rgba(224,50,82,.25)':open?'rgba(27,138,90,.18)':'#f0eadb'}`,transition:'transform .2s,box-shadow .2s'}}
      ...
      {bet.partidos?.length>0&&(
        <div style={{display:'flex',flexDirection:'column',gap:'.4rem',marginBottom:'1.1rem'}}>
          {bet.partidos.slice(0,3).map(m=>{
            ...
            return(
              <div key={m.id} style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.6rem',padding:'.55rem .75rem',borderRadius:9,background:pred?'rgba(78,140,255,.05)':'rgba(12,24,43,.02)',border:pred?'1px solid rgba(78,140,255,.2)':'1px solid #f0eadb'}}>
                ...
                <div style={{textAlign:'center',minWidth:60}}>
                  {pred?(
                    <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.05rem',color:'#0041f0',letterSpacing:'.04em'}}>{pred.pred_local} - {pred.pred_visitante}</span>
                  ):(
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',paddingTop:'.85rem',borderTop:'1px solid #f0eadb'}}>
        ...
        {canPredict&&(
          <button onClick={()=>onPredict(bet)} style={{fontWeight:700,fontSize:'.8rem',padding:'.55rem 1.1rem',borderRadius:99,border:'none',background:'#4e8cff',color:'#05090f',cursor:'pointer',transition:'all .17s',boxShadow:'0 4px 14px rgba(78,140,255,.25)',whiteSpace:'nowrap'}}
            ...
            <div className="px-5 pb-5 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  { pts: 5, titulo: 'Marcador exacto',  desc: 'Acertás el resultado final exacto del partido', accent: '#0041f0', bg: 'rgba(78,140,255,.07)',  border: 'rgba(78,140,255,.28)' },
              ...
              <div className="rounded-xl p-4" style={{ background: 'rgba(78,140,255,.06)', border: '1px solid rgba(78,140,255,.25)' }}>
                  ...
                  {[
                    ...
                    <div key={titulo} className="flex items-center gap-3 rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid #f0eadb' }}>
                      <div className="flex flex-col items-center justify-center flex-shrink-0"
                        style={{ width: 38, height: 38, borderRadius: 8, background: 'rgba(78,140,255,.1)', border: '1px solid rgba(78,140,255,.3)' }}>
                        <span className="font-display leading-none" style={{ fontSize: '1.25rem', color: '#0041f0' }}>{pts}</span>
                      </div>
