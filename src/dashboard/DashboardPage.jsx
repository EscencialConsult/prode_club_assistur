...
function StatCard({ label, value, sub, icon, gold = false, live = false }) {
  const accentColor = live ? '#e03252' : gold ? '#0041f0' : '#425b8b'
  const borderColor = live ? 'rgba(224,50,82,.2)' : gold ? 'rgba(201,159,22,.3)' : '#f0eadb'
  const iconBg      = live ? 'rgba(224,50,82,.08)' : gold ? 'rgba(78,140,255,.1)' : 'rgba(66,91,139,.08)'
  return (
...
function BetRow({ bet, onPredict }) {
  ...
  return (
    <div
      ...
      onMouseEnter={e => { e.currentTarget.style.background = '#fffdf5'; e.currentTarget.style.borderColor = '#4e8cff'; e.currentTarget.style.transform = 'translateX(3px)' }}
    ...
    >
      <span className="w-2 h-2 rounded-full flex-shrink-0"
        style={{ background: hasLive ? '#e03252' : '#4e8cff', boxShadow: hasLive ? '0 0 8px rgba(224,50,82,.5)' : '0 0 6px rgba(78,140,255,.5)' }} />
      ...
      <span className="font-body text-xs font-semibold flex-shrink-0" style={{ color: closingSoon ? '#0041f0' : '#0c182b' }}>{remaining}</span>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            <div className="text-center">
              ...
              <p className="font-display text-xl leading-none" style={{ color: '#0041f0' }}>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center rounded-2xl"
      ...
      <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3"
        style={{ background: 'rgba(78,140,255,.08)', border: '1px solid rgba(78,140,255,.2)' }}>
        <div style={{ color: '#0041f0' }}>{icon}</div>
      </div>
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      {to && (
        <Link to={to} className="font-body font-semibold text-xs flex items-center gap-1.5 transition-colors"
          style={{ color: '#0041f0', textDecoration: 'none' }}
          onMouseEnter={e => { e.currentTarget.style.color = '#0c182b' }}
          onMouseLeave={e => { e.currentTarget.style.color = '#0041f0' }}>
          {cta}
    ...
    <AppShell>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 1.5rem 3rem' }}>
        ...
        <div className="rounded-2xl p-6 md:p-8 mb-8 relative overflow-hidden animate-fade-in"
          style={{ background: '#0c182b', border: '1px solid rgba(78,140,255,.2)', boxShadow: '0 12px 40px rgba(12,24,43,.15)' }}>
          <div className="absolute top-0 right-0 w-64 h-64 pointer-events-none"
            style={{ background: 'radial-gradient(circle at 80% 20%, rgba(78,140,255,.12), transparent 65%)' }} />
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-end justify-between gap-5">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse-live" />
                <span className="font-body font-bold text-xs uppercase tracking-widest" style={{ color: 'rgba(78,140,255,.7)' }}>
                  Mundial 2026
                </span>
              </div>
              <h1 className="font-display leading-none mb-1" style={{ fontSize: 'clamp(2.2rem,6vw,3.5rem)', letterSpacing: '.02em' }}>
                ...
                <span style={{ color: '#4e8cff' }}>{nombre}</span>
              </h1>
            ...
            <div className="flex gap-2 flex-shrink-0">
              <Link to="/partidos"
                ...
                style={{ background: '#4e8cff', color: '#05090f', boxShadow: '0 6px 20px rgba(78,140,255,.3)', textDecoration: 'none' }}
              ...
              <Link to="/ranking"
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(78,140,255,.4)'; e.currentTarget.style.color = '#4e8cff' }}
              ...
              {[
                ...
                >
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: 'linear-gradient(135deg,#0c182b,#425b8b)', color: '#4e8cff' }}>
                    {icon}
                  </div>
                  <div className="flex-1 min-w-0">
