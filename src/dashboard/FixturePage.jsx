  ...
  const ESTADO = {
    ...
    finalizado: { label:'Finalizado', color:'#0041f0', bg:'rgba(78,140,255,.08)', border:'rgba(78,140,255,.22)' },
  ...
  function Chip({ active, onClick, children }) {
    return (
      <button onClick={onClick} style={{ padding:'.32rem .75rem', borderRadius:99, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:'.72rem', textTransform:'uppercase', letterSpacing:'.05em', cursor:'pointer', transition:'all .15s', background:active?'#0c182b':'#fff', color:active?'#4e8cff':'#5f6e8a', border:active?'1px solid transparent':'1px solid #f0eadb', boxShadow:active?'0 2px 8px rgba(12,24,43,.18)':'none' }}>
        {children}
      </button>
    )
  }
...
<div className="din" style={{ marginBottom:'1.5rem' }}>
  <h1 style={{
    ...
    <span style={{color:'#4e8cff'}}>MUNDIALISTA</span>
  </h1>
            ...
            <div className="din">
              <div style={{ display:'flex', flexWrap:'wrap', gap:'.6rem', marginBottom:'1.5rem' }}>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar equipo..."
                  ...
                  onFocus={e=>{ e.target.style.borderColor='rgba(78,140,255,.5)' }}
              ...
              {filtered.length===0
                ...
                : <div style={{ display:'flex', flexDirection:'column', gap:'2rem' }}>
                    {Object.entries(groups).map(([group,items])=>(
                      <div key={group}>
                        <p style={{ fontSize:'.7rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'rgba(78,140,255,.85)',marginBottom:'.7rem' }}>{group}</p>
